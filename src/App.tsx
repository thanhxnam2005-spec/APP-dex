...
import { 
  ...
  FileDown, Sparkles, Key, Settings, Headphones, Play, Pause, Volume2, Filter, Hash, Merge, Edit2, AlignLeft, Tag, Zap, Terminal, Settings2, History,
  Wrench, List, Check, ArrowUpDown
} from 'lucide-react';
...
const StoryItem = React.memo(({
  ...
  return (
    <div className="hover:bg-[#141414]/[0.02] transition-all group border-b border-[#141414]/5 last:border-0">
      <div className="p-5">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="space-y-2 cursor-pointer flex-1" onClick={() => onExpand(isExpanded ? null : story.id)}>
            <div className="flex items-center gap-2">
              <h3 className="font-mono font-bold text-base uppercase tracking-tight flex items-center gap-3">
                <div className="p-1.5 bg-[#141414] text-white">
                  <BookOpen className="w-4 h-4" />
                </div>
                {story.name}
              </h3>
              {isExpanded ? <ChevronUp className="w-4 h-4 opacity-40 shrink-0" /> : <ChevronDown className="w-4 h-4 opacity-40 shrink-0" />}
            </div>
            
            <div className="flex flex-wrap items-center gap-3 mt-1 text-[10px] font-mono">
              <span className="opacity-40 flex items-center gap-1"><History className="w-3 h-3" /> {story.date}</span>
              {story.genre && (
                <span className="bg-purple-600/10 text-purple-700 px-2 py-0.5 border border-purple-200 uppercase font-bold">
                  {story.genre}
                </span>
              )}
              <span className="px-2 py-0.5 border border-[#141414]/10 bg-[#141414]/5 opacity-60">
                {Math.round(story.content.length / 1024)} KB
              </span>
            </div>

            {story.description && (
              <p className="text-[11px] font-mono opacity-50 italic line-clamp-1 max-w-2xl border-l-2 border-[#141414]/10 pl-3 py-1">
                {story.description}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex gap-1 group/actions">
               <Button 
                ...
                className="h-8 text-[9px] font-mono uppercase hover:bg-[#141414] hover:text-white rounded-none border-2 border-[#141414] shadow-[2px_2px_0px_0px_rgba(20,20,20,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all"
              >
                {isScanningThisStory ? <RefreshCw className="w-3 h-3 animate-spin mr-1.5" /> : <Scissors className="w-3 h-3 mr-1.5" />}
                Tách chương
              </Button>
              <Button 
                size="sm" 
                ...
                className="h-8 text-[9px] font-mono uppercase hover:bg-emerald-500 hover:text-white rounded-none border-2 border-emerald-500 text-emerald-600 shadow-[2px_2px_0px_0px_rgba(16,185,129,0.3)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all"
              >
                <Zap className="w-3 h-3 mr-1.5" /> Quét lỗi
              </Button>
            </div>
            ...
            <div className="flex gap-1">
              <Button 
                ...
                className="h-8 w-8 hover:bg-green-600 hover:text-white rounded-none border border-[#141414]/20"
                title="Tải TXT"
              >
              ...
              <Button 
                ...
                className="h-8 w-8 hover:bg-red-600 hover:text-white rounded-none border border-[#141414]/20"
                title="Tải PDF"
              >
            ...
            <Button 
              ...
              className="h-8 w-8 hover:bg-red-500 hover:text-white rounded-none border border-red-200 text-red-500"
              title="Xóa truyện"
            >
        ...
        {isExpanded && (
          ...
          >
            <div className="p-4 space-y-4">
              {/* Settings & Secondary Actions Grid */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Formatting & Utilities */}
                <div className="bg-white border-2 border-[#141414] p-4 neo-brutalism-shadow space-y-4">
                  <div className="flex items-center gap-2 text-xs font-mono font-bold uppercase">
                    <Wrench className="w-4 h-4" /> TIỆN ÍCH ĐỊNH DẠNG
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <Button 
                      ...
                      className="h-9 text-[9px] font-mono uppercase bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-none border-emerald-200"
                    >
                      <Filter className="w-3 h-3 mr-2" /> Lọc dấu (.)
                    </Button>
                    <Button 
                      size="sm" 
                      variant="outline" 
                      onClick={() => onOptimizeLineBreaks(story.id)}
                      className="h-9 text-[9px] font-mono uppercase bg-cyan-50 text-cyan-700 hover:bg-cyan-100 rounded-none border-cyan-200"
                    >
                      <AlignLeft className="w-3 h-3 mr-2" /> Tối ưu dòng
                    </Button>
                    <Button 
                      size="sm" 
                      variant="outline" 
                      onClick={() => onRenumberDialog({ isOpen: true, storyId: story.id, startNumber: 1, prefix: 'Chương' })}
                      className="h-9 text-[9px] font-mono uppercase bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-none border-indigo-200"
                    >
                      <Hash className="w-3 h-3 mr-2" /> Đánh số lại
                    </Button>
                    <Button 
                      size="sm" 
                      variant="outline" 
                      onClick={() => onMergeChapters(story.id)}
                      className="h-9 text-[9px] font-mono uppercase bg-orange-50 text-orange-700 hover:bg-orange-100 rounded-none border-orange-200"
                    >
                      <Merge className="w-3 h-3 mr-2" /> Gộp chương
                    </Button>
                  </div>

                  <div className="pt-2 border-t border-[#141414]/10 space-y-2">
                    <Label className="text-[9px] font-mono uppercase opacity-50">Tiền tố tách chương tùy chỉnh</Label>
                    <div className="flex gap-2">
                      <Input 
                        placeholder="Vd: 'Hồi', 'Tiết'..."
                        value={localPrefix}
                        onChange={(e) => setLocalPrefix(e.target.value)}
                        className="h-8 text-xs border border-[#141414]/20 rounded-none font-mono flex-1"
                      />
                      <Button 
                        size="sm" 
                        onClick={() => {
                        ...
                        className="h-8 text-[9px] font-mono uppercase bg-[#141414] text-white rounded-none px-4"
                      >
                        Lưu
                      </Button>
                    </div>
                    <div className="flex items-center gap-2 bg-[#141414]/5 px-2 py-1 border border-dashed border-[#141414]/10">
                      <input 
                        type="checkbox" 
                      ...
                      />
                      <Label htmlFor={`strict-mode-${story.id}`} className="text-[8px] font-mono uppercase cursor-pointer opacity-70">Nghiêm ngặt (Tránh tách nhầm)</Label>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border-2 border-[#141414] p-4 neo-brutalism-shadow space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-xs font-mono font-bold uppercase">
                      <Sparkles className="w-4 h-4 text-purple-600" /> THÔNG TIN AI
                    </div>
                    {story.convertedChapters && Object.keys(story.convertedChapters).length > 0 && (
                      <div className="px-2 py-0.5 bg-green-100 text-green-700 border border-green-200 text-[8px] font-bold uppercase rounded-full">
                        Đã dịch AI
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="p-3 bg-purple-50 border border-purple-200 border-dashed space-y-2">
                       <p className="text-[9px] font-mono leading-relaxed text-purple-900 opacity-80">
                         {story.description || "Chưa có giới thiệu truyện. Hãy dùng tính năng lưu truyện để AI tự động phân tích và tạo giới thiệu."}
                       </p>
                    </div>

                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        <span className="text-[9px] font-mono uppercase opacity-50 font-bold">Lỗi phổ biến:</span>
                        <Button 
                          size="sm" 
                          variant="ghost" 
                          onClick={() => onEditCommonErrors(story.id)}
                          className="h-6 text-[8px] font-mono uppercase text-blue-600 hover:bg-blue-50"
                        >
                          <Edit2 className="w-2.5 h-2.5 mr-1" /> Chỉnh sửa
                        </Button>
                      </div>
                      <div className="flex flex-wrap gap-1 max-h-[80px] overflow-y-auto custom-scrollbar p-1">
                        {story.commonErrors && story.commonErrors.length > 0 ? (
                          story.commonErrors.slice(0, 10).map((err, i) => (
                            <span key={i} className="text-[8px] font-mono bg-white border border-[#141414]/10 px-1.5 py-0.5" title={`${err.o} -> ${err.n}`}>
                              {err.n}
                            </span>
                          ))
                        ) : (
                          <span className="text-[8px] font-mono opacity-30 italic">Chưa có dữ liệu quét lỗi...</span>
                        )}
                        {(story.commonErrors?.length || 0) > 10 && <span className="text-[8px] font-mono opacity-30">+{story.commonErrors!.length - 10} khác</span>}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#141414] text-[#E4E3E0] p-4 shadow-[4px_4px_0px_0px_rgba(20,20,20,0.2)]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-mono font-bold uppercase tracking-wider">
                      QUÉT TÁCH: {expandedChapters.length} CHƯƠNG
                    </h4>
                    <p className="text-[9px] font-mono opacity-60">Nhấp vào chương để xem nhanh hoặc tải về</p>
                  </div>
                </div>
                {expandedChapters.length > 0 && (
                   <div className="flex gap-2">
                     <Button 
                       ...
                       variant="ghost" 
                       onClick={() => {
                          const allErrors = story.commonErrors || [];
                          allErrors.forEach(err => onAddToGlossary(err.o, err.n));
                          toast.success(`Đã thêm ${allErrors.length} từ vào từ điển chung!`);
                       }}
                       disabled={!story.commonErrors?.length}
                       className="h-8 text-[9px] font-mono uppercase bg-white/5 hover:bg-white/20 text-white rounded-none flex items-center gap-2 px-3 border border-white/10"
                     >
                        <Library className="w-3 h-3" /> Thêm lỗi AI vào từ điển
                     </Button>
                   </div>
                )}
              </div>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 max-h-[400px] overflow-y-auto custom-scrollbar p-1">
...
export default function App() {
  ...
  useEffect(() => {
    // Check for Secure Context (Required for Web Crypto / OpenPGP)
    if (window.isSecureContext === false) {
      setIsSecureContext(false);
      console.warn("This application requires a Secure Context (HTTPS or localhost) to function correctly.");
    }
  }, []);
  ...
  const [librarySort, setLibrarySort] = useState<'date' | 'name'>('date');
  ...
  const [useOptimizeFormatting, setUseOptimizeFormatting] = useState<boolean>(true);
  const [showDiff, setShowDiff] = useState<boolean>(true);

  const GENRE_OPTIONS = ["Tiên Hiệp", "Huyền Huyễn", "Khoa Huyễn", "Đồng Nhân", "Ngôn Tình", "Đô Thị"];
  const STYLE_OPTIONS = ["Huyễn tưởng tu tiên", "Đông phương huyền huyễn", "Cổ điển tiên hiệp", "Cổ đại võ hiệp", "Hiện đại đô thị"];
  ...
  const filteredLibrary = useMemo(() => {
    ...
    if (librarySort === 'name') {
      ...
      // Default date sort is already handled in useEffect on mount, 
      // but if library changes we ensure consistency.
      result.sort((a, b) => {
        try {
  ...
  const [apiKeys, setApiKeys] = useState<string[]>(() => {
    ...
    return systemKey && systemKey !== 'undefined' ? [systemKey] : [];
  });
  ...
  const scanGeminiModels = async (key: string) => {
    ...
    try {
      ...
      // Fallback to v1 if v1beta fails with 404 or other issues
      if (!response.ok || !data.models) {
        console.warn("v1beta scan failed, trying v1...", data);
      ...
      if (data.models) {
        const models = data.models
          ...
          .filter((m: string) => m.toLowerCase().includes('gemini'));
        
        if (models.length > 0) {
          setAvailableGeminiModels(models);
          toast.success(`Đã tìm thấy ${models.length} model Gemini`);
        } else {
          toast.error("Không tìm thấy model Gemini nào cho Key này.");
        }
      } else {
        const errorMsg = data.error?.message || response.statusText || "Lỗi không xác định";
        const errorCode = data.error?.code || response.status;
        toast.error(`Lỗi API (${errorCode}): ${errorMsg}`);
        
        if (errorCode === 403) {
          toast.info("Lưu ý: API Key của bạn có thể bị hạn chế quyền truy cập model list.");
        }
      }
    } catch (error) {
      console.error("Model scan failed", error);
      toast.error(`Lỗi kết nối: ${(error as Error).message}. Vui lòng kiểm tra lại mạng hoặc Proxy.`);
    } finally {
  ...
  const [customPromptQuality, setCustomPromptQuality] = useState(`Dịch truyện {genre}, phong cách {style}.
Yêu cầu:
- Sát gốc 100%, không thừa thiếu.
- Dùng từ ngữ CỔ PHONG, KIẾM HIỆP phù hợp bối cảnh {genre}. TUYỆT ĐỐI không dùng từ ngữ hiện đại (Vd: tránh dùng "liên hệ", "vấn đề", "phản ứng", "cảm xúc"... nếu không hợp).
- Văn phong tự nhiên, thuần Việt (Vd: "dựa theo" thay vì "dựa trên").
- Xưng hô {genre} mượt mà. Ngắt đoạn đẹp.
- KHÔNG giải thích. CHỈ trả về văn bản dịch.`);

  const [customPromptFast, setCustomPromptFast] = useState(`Dịch nhanh truyện {genre}.
Yêu cầu:
- Sát gốc 100%, không thừa.
- Giữ đúng chất truyện {genre}, tránh dùng từ hiện đại lạc quẻ.
- Dịch mượt, thuần Việt (Vd: "dựa theo" thay vì "dựa trên").
- Sửa Hán Việt sang từ thông dụng nhưng vẫn giữ phong thái cổ trang.
- KHÔNG giải thích. CHỈ trả về văn bản dịch.`);

  const [customPromptProofread, setCustomPromptProofread] = useState(`Sửa lỗi và tối ưu văn bản {genre}.
Yêu cầu:
- Sửa lỗi chính tả, converter, sát gốc 100%.
- Văn phong CỔ PHONG tự nhiên, mượt mà. Tránh từ hiện đại.
- Ngắt câu, xuống dòng rõ ràng, ngay ngắn.
- KHÔNG giải thích. CHỈ trả về văn bản xử lý.`);
  ...
  const handleExport = async (scope: 'all' | 'range', format: 'zip' | 'single' | 'bundle20') => {
    ...
    if (format === 'bundle20') {
      ...
      for (let i = 0; i < selectedChapters.length; i += BUNDLE_SIZE) {
        const bundleChapters = selectedChapters.slice(i, i + BUNDLE_SIZE);
        const startNumValue = i + startIdx + 1;
        const endNumValue = Math.min(i + startIdx + BUNDLE_SIZE, startIdx + selectedChapters.length);

        const bundleContent = bundleChapters.map(ch => {
        ...
        const fileName = `${story.name}_ch${startNumValue}_to_${endNumValue}${suffix}.txt`;
        zip.file(fileName, bundleContent);
      }

      const content = await zip.generateAsync({ type: 'blob' });
      const rangeSuffix = scope === 'range' ? `_ch${startIdx + 1}_to_${endIdx}` : '';
      saveAs(content, `${story.name}${suffix}${rangeSuffix}_grouped_20.zip`);
      return;
    }
  ...
  const processTextWithAI = async (text: string, onProgress: (progress: number) => void, systemInstructionOverride?: string, onCorrectionsFound?: (corrections: {o: string, n: string}[]) => void, storyErrors?: {o: string, n: string}[]) => {
    ...
    const getAIInstance = (index: number) => {
      if (aiProvider === 'gemini') {
        const systemKey = process.env.GEMINI_API_KEY;
        const key = apiKeys[index] || (systemKey && systemKey !== 'undefined' ? systemKey : '');
        if (!key) throw new Error("Không tìm thấy API Key. Vui lòng cấu hình trong phần Cài đặt.");
    ...
    const processChunk = async (chunk: string, index: number, retryCount = 0): Promise<void> => {
      ...
      try {
        ...
        if (!systemInstructionOverride) {
          const genreStr = selectedGenres.length > 0 ? selectedGenres.join(", ") : "Truyện";
          const styleStr = selectedStyles.length > 0 ? selectedStyles.join(", ") : "";
          
          // Base instructions requested by user:
          let basePrompt = `Nhiệm vụ: Biên dịch văn bản bám sát bản gốc 100% tuyệt đối, không thừa không thiếu bất kỳ từ ngữ hay đoạn nào.
Quy tắc:
- TUYỆT ĐỐI không giải thích, không thêm thắt lời dẫn hay Meta-commentary.
- Tối ưu hóa token: Chỉ trả về duy nhất văn bản kết quả sau biên dịch.
- Văn phong tự nhiên, thuần Việt (Ví dụ: dùng "dựa theo tập tục" thay vì "dựa trên tập tục").
- Giữ nguyên góc nhìn nhân vật (POV) của bản gốc 100%.`;

          if (selectedGenres.length > 0) {
            basePrompt += `\n- Thể loại: ${genreStr}. Hãy dùng từ ngữ đặc thù phù hợp với thể loại này.`;
          }
          if (selectedStyles.length > 0) {
            basePrompt += `\n- Phong cách dịch: ${styleStr}.`;
          }
          if (useOptimizeFormatting) {
            basePrompt += `\n- Tối ưu văn bản: Ngắt câu và xuống dòng hợp lý, dễ nhìn, dễ đọc. Xoá các ký tự thừa thãi nhưng TUYỆT ĐỐI không xoá hay thêm nội dung truyện.`;
          }

          if (translationMode === 'fast') {
            systemInstruction = `${basePrompt}\n- Ưu tiên: Dịch nhanh, mượt mà, thuần Việt.`;
          } else if (translationMode === 'proofread') {
            systemInstruction = `${basePrompt}\n- Ưu tiên: Sửa lỗi chính tả, lỗi converter, làm văn trôi chảy tự nhiên.`;
          } else {
            systemInstruction = `${basePrompt}\n- Ưu tiên: Dịch chất lượng cao, văn chương sâu sắc, đúng sắc thái nguyên tác.`;
          }
        }
        ...
        if (aiProvider === 'gemini') {
          const ai = currentAI as GoogleGenAI;
          const result = await ai.models.generateContent({ 
            model: geminiModel,
            contents: chunk,
            config: {
          ...
          const estimatedTokens = Math.ceil((chunk.length + responseText.length) / 4);
...
Yêu cầu trả về định dạng JSON như sau:
{
  ...
  "description": "Đoạn giới thiệu hấp dẫn"
      ...
      if (aiProvider === 'gemini') {
        const systemKey = process.env.GEMINI_API_KEY;
        const key = apiKeys[currentKeyIndexRef.current] || (systemKey && systemKey !== 'undefined' ? systemKey : '');
        if (!key) throw new Error("Không có API Key. Vui lòng cấu hình trong phần Cài đặt.");
        const ai = new GoogleGenAI({ apiKey: key });
        const result = await ai.models.generateContent({ 
          model: geminiModel,
          contents: prompt
  ...
  return (
    <div className="min-h-screen bg-[#E4E3E0] text-[#141414] font-sans p-4 md:p-8 flex items-center justify-center">
      <div className="max-w-3xl w-full space-y-8">
        {!isSecureContext && (
          <div className="bg-red-600 text-white p-4 font-mono text-xs border-2 border-[#141414] shadow-[4px_4px_0px_0px_rgba(20,20,20,1)] animate-pulse">
            <div className="flex items-center gap-2 mb-2 font-bold uppercase">
              <AlertCircle className="w-4 h-4" /> CẢNH BÁO AN NINH (SECURITY WARNING)
            </div>
            Ứng dụng đang chạy trên kết nối không bảo mật (không phải HTTPS). Các chức năng giải mã GPG có thể không hoạt động do hạn chế của trình duyệt. 
            Vui lòng triển khai ứng dụng trên HTTPS hoặc chạy tại localhost.
          </div>
        )}
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              const customModel = prompt("Nhập tên model thủ công (VD: gemini-3-flash-preview):");
                              if (customModel) {
                                setAvailableGeminiModels(prev => [...new Set([...prev, customModel])]);
                                setGeminiModel(customModel);
                                toast.success("Đã thêm model thủ công");
                              }
                            }}
                            title="Thêm model thủ công"
                          >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-2">
                  <div className="relative group">
                    ...
                    <Input 
                      placeholder="Tìm kiếm truyện, giới thiệu hoặc thể loại..."
                      value={librarySearch}
                      onChange={(e) => setLibrarySearch(e.target.value)}
                      className="pl-10 border-[#141414] focus-visible:ring-0 focus-visible:border-2 rounded-none font-mono h-12 bg-white/50 backdrop-blur-sm shadow-sm"
                    />
                  </div>
                </div>
                <div>
                  <div className="flex items-center h-12 bg-white/50 border border-[#141414] px-3 font-mono text-xs gap-2">
                    <span className="opacity-40 uppercase">Sắp xếp:</span>
                    <select 
                      value={librarySort}
                      onChange={(e) => setLibrarySort(e.target.value as any)}
                      className="flex-1 bg-transparent outline-none cursor-pointer font-bold"
                    >
                ...
                <div className="flex items-center justify-end">
                   <p className="font-mono text-[10px] opacity-40 uppercase">
                    Tổng số: <span className="font-bold text-[#141414] opacity-100">{filteredLibrary.length} bộ</span>
                   </p>
                </div>
              </div>

              <Card className="border-[#141414] border-2 shadow-none bg-white neo-brutalism-shadow overflow-hidden">
                <CardHeader className="border-b border-[#141414] bg-[#141414]/5 pb-4">
                  <CardTitle className="text-lg font-mono flex items-center gap-2">
                    <Library className="w-4 h-4" />
                    KHO TRUYỆN ĐÃ LƯU TRỮ
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="h-[600px] overflow-y-auto custom-scrollbar technical-grid">
                    {filteredLibrary.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full opacity-20 space-y-4">
                        <BookOpen className="w-16 h-16" />
                        <p className="font-mono uppercase tracking-widest text-sm text-center px-6">
                          {library.length === 0 ? "Kho lưu trữ đang trống" : "Không tìm thấy kết quả phù hợp"}
                        </p>
                      </div>
                    ) : (
          ...
          <TabsContent value="convert" className="mt-6 space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              {/* Left Column: Selection and Settings (Bento Cards) */}
              <div className="lg:col-span-4 space-y-6">
                {/* Story Selection Card */}
                <Card className="border-[#141414] border-2 shadow-none neo-brutalism-shadow bg-white">
                  <CardHeader className="border-b border-[#141414] py-3 bg-[#141414]/5">
                    <CardTitle className="text-sm font-mono flex items-center gap-2">
                       <BookOpen className="w-3 h-3" /> CHỌN NGUỒN
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="pt-4 space-y-4">
                    <div className="space-y-2">
                      <Label className="text-[10px] font-mono uppercase opacity-50">Truyện trong kho</Label>
                      <select 
                        ...
                        className="w-full bg-white border-2 border-[#141414] h-10 px-3 font-mono text-xs outline-none focus:bg-[#141414]/5 transition-colors"
                      >
                    ...
                    {selectedConvertStoryId && (
                      <div className="flex flex-col gap-2 pt-2">
                        <Button 
                          ...
                          className="h-9 font-mono text-[10px] uppercase bg-green-600 hover:bg-green-700 text-white rounded-none w-full"
                        >
                          <FileText className="w-3 h-3 mr-2" /> Tải TXT
                        </Button>
                        <div className="grid grid-cols-2 gap-2">
                          <Button 
                            size="sm"
                            ...
                            className="h-9 font-mono text-[10px] uppercase bg-red-600 hover:bg-red-700 text-white rounded-none"
                          >
                            <FileDown className="w-3 h-3 mr-2" /> Tải PDF
                          </Button>
                          <Button 
                            size="sm"
                            onClick={() => {
                            ...
                            className="h-9 font-mono text-[10px] uppercase bg-blue-600 hover:bg-blue-700 text-white rounded-none"
                          >
                            <FileText className="w-3 h-3 mr-2" /> WORD
                          </Button>
                        </div>
                      </div>
                    )}
                ...
                <Card className="border-[#141414] border-2 shadow-none neo-brutalism-shadow bg-white overflow-hidden">
                  <CardHeader className="border-b border-[#141414] py-3 bg-purple-600 text-white">
                    <CardTitle className="text-sm font-mono flex items-center gap-2">
                       <Settings2 className="w-3 h-3" /> CẤU HÌNH AI
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="pt-4 space-y-4">
                    <div className="space-y-3">
                      <div className="space-y-1">
                        <Label className="text-[10px] font-mono uppercase opacity-50">Thể loại</Label>
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                selectedGenres.includes(genre) 
                                  ? "bg-[#141414] text-white border-[#141414] translate-x-[2px] translate-y-[2px] shadow-none" 
                                  : "bg-white text-[#141414] border-[#141414] hover:bg-[#141414]/5 shadow-[2px_2px_0px_0px_rgba(20,20,20,1)]"
                              )}
                            >
                              {genre}
                            </button>
                      ...
                      <div className="space-y-1">
                        <Label className="text-[10px] font-mono uppercase opacity-50">Phong cách</Label>
                        <div className="flex flex-wrap gap-1">
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                selectedStyles.includes(style) 
                                  ? "bg-purple-600 text-white border-purple-600 translate-x-[2px] translate-y-[2px] shadow-none" 
                                  : "bg-white text-[#141414] border-[#141414] hover:bg-purple-50 shadow-[2px_2px_0px_0px_rgba(20,20,20,1)]"
                              )}
                            >
                              {style}
                            </button>
                      ...
                      <div className="pt-2 border-t border-[#141414]/10 space-y-3">
                        <div className="flex items-center justify-between">
                          <Label className="text-[10px] font-mono uppercase opacity-50">Tối ưu văn bản</Label>
                          <button 
                            onClick={() => setUseOptimizeFormatting(!useOptimizeFormatting)}
                            className={cn(
                              "relative inline-flex h-5 w-10 items-center transition-colors focus:outline-none border border-[#141414]",
                              useOptimizeFormatting ? "bg-green-500" : "bg-gray-200"
                            )}
                          >
                            <span
                              className={cn(
                                "inline-block h-3 w-3 transform transition-transform border border-[#141414] bg-white",
                                useOptimizeFormatting ? "translate-x-6" : "translate-x-1"
                              )}
                            />
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <Label className="text-[10px] font-mono uppercase opacity-50">Chế độ dịch</Label>
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            onClick={() => setShowPromptSettings(true)}
                            className="h-6 w-6 rounded-none hover:bg-[#141414] hover:text-white"
                          >
                        ...
                        <select 
                          ...
                          className="w-full h-9 text-[11px] font-mono bg-white border-2 border-[#141414] px-2 outline-none"
                        >
                          <option value="quality">Dịch Chất lượng cao</option>
                          <option value="fast">Dịch siêu tốc</option>
                          <option value="proofread">Sửa lỗi (Proofread)</option>
                        </select>
                      </div>

                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                          <Label className="text-[10px] font-mono uppercase opacity-50">Song song</Label>
                          <select 
                            value={concurrency}
                            onChange={(e) => setConcurrency(parseInt(e.target.value))}
                            className="w-full h-8 text-[11px] font-mono bg-white border border-[#141414]/20 px-2 rounded-sm"
                          >
                        ...
                        <div className="space-y-1">
                          <Label className="text-[10px] font-mono uppercase opacity-50">Bỏ qua tới</Label>
                          <Input 
                            type="number" 
                            min="0"
                            value={skipChaptersCount}
                            onChange={(e) => setSkipChaptersCount(parseInt(e.target.value) || 0)}
                            className="h-8 w-full text-[11px] font-mono rounded-sm border-[#141414]/20"
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <Card className="border-[#141414] border-2 shadow-none neo-brutalism-shadow bg-white">
                    <CardHeader className="border-b border-[#141414] py-3 bg-[#141414]/5">
                      <CardTitle className="text-sm font-mono flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <List className="w-3 h-3" /> DANH SÁCH CHƯƠNG
                        </div>
                        <span className="text-[9px] opacity-40 uppercase">{currentConvertChapters.length} mục</span>
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="p-0">
                      <div className="h-[400px] overflow-y-auto custom-scrollbar">
                        <div className="divide-y divide-[#141414]/5">
                          {currentConvertChapters.map((ch, idx) => {
                            ...
                            const isSelected = selectedConvertChapterIdx === idx;
                            return (
                              <div 
                                key={`conv-ch-${idx}`}
                                onClick={() => setSelectedConvertChapterIdx(idx)}
                                className={cn(
                                  "p-2.5 cursor-pointer transition-all flex items-center justify-between group",
                                  isSelected ? "bg-[#141414] text-white" : "hover:bg-[#141414]/5 border-l-2 border-transparent",
                                  isSelected && "border-l-purple-500"
                                )}
                              >
                                <div className="flex flex-col overflow-hidden">
                                  <span className={cn("text-[8px] font-mono uppercase mb-0.5", isSelected ? "text-purple-300" : "opacity-40")}>Chương {idx + 1}</span>
                                  <span className="text-[11px] font-mono font-bold truncate tracking-tight">{ch.title}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  {isConverted && (
                                    <div className="bg-green-500 text-white p-0.5 rounded-full shadow-[0_0_5px_rgba(34,197,94,0.5)]">
                                      <Check className="w-2 h-2" />
                                    </div>
                                  )}
                                  {!isConverted && idx >= skipChaptersCount && !isSelected && (
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      onClick={(e) => {
              ...
              <div className="lg:col-span-8 space-y-4">
                {selectedConvertChapterIdx !== null ? (
                   <div className="space-y-4 h-full flex flex-col">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border-2 border-[#141414] p-4 neo-brutalism-shadow">
                        <div className="space-y-1">
                          <h2 className="text-sm font-mono font-bold uppercase tracking-wider">
                            ĐANG XỬ LÝ: {currentConvertChapters[selectedConvertChapterIdx]?.title}
                          </h2>
                          <div className="flex items-center gap-4 text-[10px] font-mono opacity-60">
                            <span className="flex items-center gap-1"><Terminal className="w-3 h-3" /> MODE: {translationMode.toUpperCase()}</span>
                            <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> CONCURRENCY: {concurrency}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          {isAutoConverting ? (
                            <Button 
                              ...
                              className="h-10 text-[10px] font-mono uppercase bg-red-600 hover:bg-red-700 text-white rounded-none px-6 shadow-[4px_4px_0px_0px_rgba(153,27,27,0.3)]"
                            >
                              <Square className="w-3 h-3 mr-2" /> DỪNG DỊCH
                            </Button>
                          ) : (
                            <Button 
                              size="sm"
                              ...
                              className="h-10 text-[10px] font-mono uppercase bg-purple-600 hover:bg-purple-700 text-white rounded-none px-6 shadow-[4px_4px_0px_0px_rgba(147,51,234,0.3)] disabled:opacity-50"
                            >
                              <RefreshCw className="w-3 h-3 mr-2" /> DỊCH TỰ ĐỘNG
                            </Button>
                          )}
                          <Button 
                            size="sm"
                            ...
                            className="h-10 text-[10px] font-mono uppercase bg-[#141414] text-white hover:bg-black rounded-none px-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)] disabled:opacity-50"
                          >
                            {isProcessingPart ? <RefreshCw className="w-3 h-3 mr-2 animate-spin" /> : <Sparkles className="w-3 h-3 mr-2" />}
                            {isProcessingPart ? `DỊCH ${partProgress}%` : 'DỊCH LẺ'}
                          </Button>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
                        {/* Original Section */}
                        <div className="flex flex-col border-2 border-[#141414] bg-[#F9F8F6] shadow-sm overflow-hidden h-[600px]">
                          <div className="bg-[#141414] text-white px-3 py-2 text-[10px] font-mono uppercase flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <FileText className="w-3 h-3" /> NGUỒN (CONVERTER)
                            </div>
                            <span className="opacity-40 text-[8px]">{currentConvertChapters[selectedConvertChapterIdx]?.content.length} ký tự</span>
                          </div>
                          <div className="flex-1 p-5 overflow-auto custom-scrollbar font-mono text-xs leading-relaxed text-[#141414] selection:bg-purple-200">
                             <pre className="whitespace-pre-wrap">{currentConvertChapters[selectedConvertChapterIdx]?.content || ""}</pre>
                          </div>
                        </div>

                        {/* Result Section */}
                        <div className="flex flex-col border-2 border-[#141414] bg-white shadow-sm overflow-hidden h-[600px]">
                          <div className="bg-purple-600 text-white px-3 py-2 text-[10px] font-mono uppercase flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <Sparkles className="w-3 h-3" /> KẾT QUẢ (AI)
                              <div className="flex items-center gap-2 ml-4">
                                <input 
                                  type="checkbox" 
                                  id="show-diff-toggle-new"
                                  checked={showDiff}
                                  onChange={(e) => setShowDiff(e.target.checked)}
                                  className="w-3 h-3 accent-yellow-400"
                                />
                                <Label htmlFor="show-diff-toggle-new" className="text-[8px] font-mono uppercase cursor-pointer">So sánh</Label>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                               {currentConvertStory?.convertedChapters?.[selectedConvertChapterIdx] && (
                                  <Button 
                                    variant="ghost" 
                                    size="icon" 
                                    className="h-6 w-6 text-white hover:bg-white/20"
                                    onClick={() => {
                                      const text = currentConvertStory?.convertedChapters?.[selectedConvertChapterIdx];
                                      if (text) downloadChapter(`${currentConvertChapters[selectedConvertChapterIdx].title}_converted`, text);
                                    }}
                                  >
                                    <Download className="w-3 h-3" />
                                  </Button>
                          ...
                          <div className="flex-1 p-5 overflow-auto custom-scrollbar font-mono text-xs leading-relaxed selection:bg-purple-100">
                             {(() => {
                               const converted = currentConvertStory?.convertedChapters?.[selectedConvertChapterIdx];
                               if (!converted) return (
                                 <div className="h-full flex flex-col items-center justify-center opacity-20 space-y-2">
                                   <RefreshCw className="w-8 h-8" />
                                   <p className="tracking-widest uppercase text-[10px]">Đang chờ biên dịch...</p>
                                 </div>
                               );
                  ...
                  <div className="h-full min-h-[600px] flex flex-col items-center justify-center border-4 border-dashed border-[#141414]/10 bg-[#141414]/5 space-y-6 group">
                     <div className="relative">
                        <BookOpen className="w-20 h-20 opacity-10 group-hover:scale-110 transition-transform duration-500" />
                        <motion.div 
                          animate={{ rotate: 360 }} 
                          transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
                          className="absolute -inset-4 border-2 border-dashed border-[#141414]/10 rounded-full" 
                        />
                     </div>
                     <div className="text-center space-y-1">
                       <p className="font-mono uppercase tracking-[0.3em] text-xs opacity-40">Hệ thống sẵn sàng</p>
                       <p className="font-mono text-[10px] opacity-30 italic">Vui lòng chọn hoặc click vào một chương để bắt đầu</p>
                     </div>
                  </div>
                )}
              </div>
      ...
      <AnimatePresence>
                ...
                <div className="space-y-4">
                  <div className="border border-[#141414]/20 p-4 space-y-3 bg-[#141414]/5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      {exportDialog.type === 'txt' && (
                        <Button 
                          onClick={() => handleExport('all', 'bundle20')}
                          className="col-span-2 border-2 border-[#141414] text-[#141414] hover:bg-[#141414] hover:text-white rounded-none text-[10px] font-mono uppercase h-9"
                        >
                          Gộp 20 chương/file (ZIP)
                        </Button>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="border border-[#141414]/20 p-4 space-y-3 bg-[#141414]/5">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      ...
                      {exportDialog.type === 'txt' && (
                        <Button 
                          onClick={() => handleExport('range', 'bundle20')}
                          className="col-span-2 border-2 border-[#141414] text-[#141414] hover:bg-[#141414] hover:text-white rounded-none text-[10px] font-mono uppercase h-8"
                        >
                          Gộp 20 chương/file (ZIP)
                        </Button>
                      )}
        ...
        {showPromptSettings && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-[#141414]/80 backdrop-blur-sm z-[130] flex items-center justify-center p-4"
          >
            <div className="bg-white border-2 border-[#141414] p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto custom-scrollbar space-y-6 shadow-[8px_8px_0px_0px_rgba(20,20,20,1)]">
              <div className="flex items-center justify-between border-b border-[#141414]/10 pb-4">
                <h3 className="text-lg font-mono font-bold uppercase flex items-center gap-2">
                  <Terminal className="w-5 h-5" />
                  Cấu hình Prompt Hệ thống
                </h3>
                <Button variant="ghost" size="icon" onClick={() => setShowPromptSettings(false)} className="h-8 w-8 rounded-none hover:bg-[#141414] hover:text-white">
                  <X className="w-4 h-4" />
                </Button>
              </div>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-mono uppercase font-bold text-purple-600 flex items-center gap-2">
                    <Sparkles className="w-3 h-3" />
                    1. Prompt - Dịch Chất lượng cao
                  </Label>
                  <p className="text-[9px] font-mono opacity-50 italic">Sử dụng: {"{genre}"}, {"{style}"} làm placeholder.</p>
                  <textarea 
                    value={customPromptQuality}
                    onChange={(e) => setCustomPromptQuality(e.target.value)}
                    className="w-full h-32 p-3 font-mono text-[10px] border border-[#141414]/20 rounded-none focus:outline-none focus:border-[#141414] resize-none"
                  />
                </div>

                <div className="space-y-2">
                  <Label className="text-[10px] font-mono uppercase font-bold text-emerald-600 flex items-center gap-2">
                    <Zap className="w-3 h-3" />
                    2. Prompt - Dịch Siêu tốc (Tiết kiệm)
                  </Label>
                  <textarea 
                    value={customPromptFast}
                    onChange={(e) => setCustomPromptFast(e.target.value)}
                    className="w-full h-32 p-3 font-mono text-[10px] border border-[#141414]/20 rounded-none focus:outline-none focus:border-[#141414] resize-none"
                  />
                </div>

                <div className="space-y-2">
                  <Label className="text-[10px] font-mono uppercase font-bold text-blue-600 flex items-center gap-2">
                    <History className="w-3 h-3" />
                    3. Prompt - Sửa lỗi trực tiếp
                  </Label>
                  <textarea 
                    value={customPromptProofread}
                    onChange={(e) => setCustomPromptProofread(e.target.value)}
                    className="w-full h-32 p-3 font-mono text-[10px] border border-[#141414]/20 rounded-none focus:outline-none focus:border-[#141414] resize-none"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-[#141414]/10">
                <Button 
                  variant="ghost" 
                  onClick={() => {
                    setCustomPromptQuality(`Dịch truyện {genre}, phong cách {style}.\nYêu cầu:\n- Sát gốc 100%, không thừa thiếu.\n- Dùng từ ngữ CỔ PHONG, KIẾM HIỆP phù hợp bối cảnh {genre}. TUYỆT ĐỐI không dùng từ ngữ hiện đại (Vd: tránh dùng "liên hệ", "vấn đề", "phản ứng", "cảm xúc"... nếu không hợp).\n- Văn phong tự nhiên, thuần Việt (Vd: "dựa theo" thay vì "dựa trên").\n- Xưng hô {genre} mượt mà. Ngắt đoạn đẹp.\n- KHÔNG giải thích. CHỈ trả về văn bản dịch.`);
                    setCustomPromptFast(`Dịch nhanh truyện {genre}.\nYêu cầu:\n- Sát gốc 100%, không thừa.\n- Giữ đúng chất truyện {genre}, tránh dùng từ hiện đại lạc quẻ.\n- Dịch mượt, thuần Việt (Vd: "dựa theo" thay vì "dựa trên").\n- Sửa Hán Việt sang từ thông dụng nhưng vẫn giữ phong thái cổ trang.\n- KHÔNG giải thích. CHỈ trả về văn bản dịch.`);
                    setCustomPromptProofread(`Sửa lỗi và tối ưu văn bản {genre}.\nYêu cầu:\n- Sửa lỗi chính tả, converter, sát gốc 100%.\n- Văn phong CỔ PHONG tự nhiên, mượt mà. Tránh từ hiện đại.\n- Ngắt câu, xuống dòng rõ ràng, ngay ngắn.\n- KHÔNG giải thích. CHỈ trả về văn bản xử lý.`);
                ...
                <Button 
                  ...
                  className="bg-[#141414] text-white hover:bg-[#141414]/90 rounded-none text-[10px] font-mono uppercase px-8"
                >
                  Đóng & Lưu
                </Button>
              </div>
